      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-hide px-2 py-2">
        {items.map((item) => (
          <button
            ...
            className={`min-h-11 shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full border shadow-sm text-xs sm:text-sm font-black active:scale-95 transition-all ${activeId === item.id
        ...
        {showBackToTop && (
          <button
            ...
            aria-label="返回本頁頂部"
            title="返回頂部"
            className="min-w-11 min-h-11 shrink-0 inline-flex items-center justify-center rounded-full border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 text-gray-700 dark:text-gray-200 hover:border-primary/40 hover:text-primary transition-colors"
          >
